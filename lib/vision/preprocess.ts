...
import { BoundingBox, FourSlot, ImageDataLike } from "../types";
...
function attenuateBackground(data: Uint8ClampedArray, width: number, height: number) {
...
function generateSyntheticImage(buffer: Buffer, width: number, height: number): ImageDataLike {
...
function clampBoundingBox(width: number, height: number, box: BoundingBox): BoundingBox {
...
function computeVisibility(img: ImageDataLike, box: BoundingBox): "ok" | "low" {
...
export async function normalizeInput(
  image: Buffer | string,
): Promise<{ img: ImageDataLike; scale: number }> {
 ...
 */
export async function locateFourSlots(img: ImageDataLike): Promise<{
  coiffe: BoundingBox;
  cape: BoundingBox;
  bouclier: BoundingBox;
  familier: BoundingBox;
  ...
  const visibility: Record<FourSlot, "ok" | "low"> = {
