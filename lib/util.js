...
function findDofusbookLinks(text){
  ...
  const normalizeCandidate = (candidate) => {
    ...
    let working = cleanWrapper(candidate);
    working = safeDecode(working);
    working = cleanWrapper(working);
    if (!working) return null;

    if (/youtube\.com\/redirect/i.test(working)) {
      try {
        const redirectUrl = new URL(working);
        const params = [
          redirectUrl.searchParams.get("q"),
          redirectUrl.searchParams.get("url"),
          redirectUrl.searchParams.get("u"),
        ];
        for (const param of params) {
          if (!param || param === working) continue;
          const normalized = normalizeCandidate(param);
    ...
    const decoded = safeDecode(working);
    working = cleanWrapper(decoded);

    if (!/dofusbook\.net|d-bk\.net/i.test(working)) return null;

    let final = working;
    if (!/^https?:/i.test(final)) {
      final = `https://${final.replace(/^\/+/, "")}`;
    }
    final = final.replace(/https?:\/\/(?:www\.)?d-bk\.net/gi, (match) => {
      return match.startsWith("http://") ? "http://dofusbook.net" : "https://dofusbook.net";
    });
    ...
    try {
      ...
      const hash = u.hash || "";
      return `https://dofusbook.net${path}${search}${hash}`;
    } catch {
      return null;
    }
  };
