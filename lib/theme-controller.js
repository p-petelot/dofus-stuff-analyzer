...
export const THEME_STORAGE_KEY = "kroskin.theme";
...
const STATIC_THEME_TOKENS = {
  ...
  [THEME_KEYS.LIGHT]: {
    ...
    "--highlight": "#8b5cf6",
    "--highlight-strong": "#6366f1",
    "--shadow": "0 22px 48px -28px rgba(15, 23, 42, 0.28)",
    ...LIGHT_ACCENT_VALUES,
...
export function buildIntelligentThemeTokens(palette) {
  ...
  const logoPrimaryHex = adjustHexLightness(baseHex, -0.42, -0.18);
  const logoSecondaryHex = adjustHexLightness(baseHex, -0.18, 0.06);
  const logoDepthHex = adjustHexLightness(baseHex, -0.52, -0.22);
  const logoHighlightHex = adjustHexLightness(secondaryAccentHex ?? baseHex, 0.22, -0.08);
  const logoHighlightStrongHex = adjustHexLightness(secondaryAccentHex ?? baseHex, 0.08, -0.2);
  ...
  baseTokens["--logo-primary"] =
    logoPrimaryHex ?? fallback["--logo-primary"] ?? "#0d3a29";
  baseTokens["--logo-secondary"] =
    logoSecondaryHex ?? fallback["--logo-secondary"] ?? "#13845b";
  baseTokens["--logo-depth"] = logoDepthHex ?? fallback["--logo-depth"] ?? "#05251b";
  baseTokens["--logo-highlight"] =
    logoHighlightHex ?? fallback["--logo-highlight"] ?? "#f3cf6b";
  baseTokens["--logo-highlight-strong"] =
    logoHighlightStrongHex ?? fallback["--logo-highlight-strong"] ?? "#c9982e";
  baseTokens["--logo-shadow"] =
    fallback["--logo-shadow"] ?? "rgba(6, 13, 19, 0.55)";
