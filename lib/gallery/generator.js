...
function hashStringToSeed(input) {
  const value = String(input ?? "gallery");
...
function createDeterministicRng(seedInput) {
  let state = hashStringToSeed(seedInput);
  return () => {
...
function pickDeterministic(array, rng) {
...
const REQUIRED_SLOTS = ["coiffe", "cape", "bouclier", "costume", "epauliere", "ailes"];
const OPTIONAL_SLOTS = ["familier"];
const CORE_SLOTS = new Set([...REQUIRED_SLOTS, ...OPTIONAL_SLOTS]);
...
const TONE_FILTERS = Object.freeze({
  blue: {
    label: "Bleu",
    seeds: ["#2A9D8F", "#38BDF8", "#3B82F6", "#1C2541", "#5BC0BE"],
    hue: [185, 235],
  },
  teal: {
    label: "Turquoise",
    seeds: ["#0EA5A6", "#0FA9C9", "#119FA5", "#36BEFA"],
    hue: [165, 185],
  },
  green: {
    label: "Vert",
    seeds: ["#2E7D32", "#3BA272", "#4CAF50", "#6DA34D"],
    hue: [95, 150],
  },
  orange: {
    label: "Orange",
    seeds: ["#F97316", "#F59E0B", "#FB923C", "#F4A261"],
    hue: [20, 55],
  },
  red: {
    label: "Rouge",
    seeds: ["#E53935", "#E76F51", "#DC2626", "#C81D25"],
    hue: [345, 360],
    wrapLow: 0,
  },
  purple: {
    label: "Violet",
    seeds: ["#7C3AED", "#9F86C0", "#67568C", "#8B5CF6"],
    hue: [260, 305],
  },
  neutral: {
    label: "Neutre",
    seeds: ["#111827", "#6B7280", "#9CA3AF", "#E5E7EB"],
    hue: null,
  },
});
...
function buildDesignerPalette(referenceHex) {
  const reference = normalizeColorToHex(referenceHex) ?? "#2A9D8F";
  const tonalPivots = [
    reference,
    adjustHexColor(reference, { l: -6, s: -6 }),
    adjustHexColor(reference, { l: 8, s: -4 }),
    adjustHexColor(reference, { h: 8, s: 6, l: 4 }),
  ].filter(Boolean);
  const analogous = deriveAnalogousPalette(reference).slice(0, 3);
  const seeds = [...tonalPivots, ...analogous, ...COHESIVE_NEUTRALS.slice(0, 2)];
  const cohesive = buildToneLockedPalette(seeds, COHESIVE_NEUTRALS);
  return { referenceColor: reference, palette: cohesive.hex };
}

function normalizeToneFilter(value) {
  if (!value || typeof value !== "string") return null;
  const key = value.trim().toLowerCase();
  return Object.prototype.hasOwnProperty.call(TONE_FILTERS, key) ? key : null;
}

function pickToneSeed(tone, rng = createDeterministicRng(tone || "tone")) {
  const normalized = normalizeToneFilter(tone);
  if (!normalized) {
    const palette = pickPalette(rng);
    const randomColor = pickDeterministic(palette, rng);
    return normalizeColorToHex(randomColor) ?? "#2A9D8F";
  }

  const preset = TONE_FILTERS[normalized];
  const candidates = Array.isArray(preset?.seeds) ? preset.seeds : [];
  if (candidates.length) {
    return normalizeColorToHex(pickDeterministic(candidates, rng)) ?? "#2A9D8F";
  }
  const palette = pickPalette(rng);
  const filtered = palette
    .map((hex) => ({ hex: normalizeColorToHex(hex), hsl: rgbToHsl(hexToRgb(hex)) }))
    .filter((entry) => entry.hex && entry.hsl);

  const [minHue, maxHue] = preset?.hue ?? [];
  const wrapLow = preset?.wrapLow ?? null;
  const pool = filtered.filter(({ hsl }) => {
    if (!hsl) return false;
    if (minHue === null || maxHue === null) return true;
    if (wrapLow === null) {
      return hsl.h >= minHue && hsl.h <= maxHue;
    }
    return hsl.h >= minHue || hsl.h <= wrapLow;
  });
  const choice = pool.length ? pickDeterministic(pool, rng) : filtered[0];
  return choice?.hex ?? "#2A9D8F";
}

function buildCreativePaletteRoles(referenceHex, toneFilter = null, rng = createDeterministicRng(referenceHex)) {
  const reference = normalizeColorToHex(referenceHex) ?? pickToneSeed(toneFilter, rng);
  const primary = reference;
  const secondary = adjustHexColor(reference, { l: -8, s: -8 }) ?? reference;
  const accent = adjustHexColor(reference, { h: 10, l: 8 }) ?? secondary;
  const neutrals = ["#111827", "#F8F9FA", "#E5E7EB", "#1F2937"];
  const paletteHex = buildToneLockedPalette([primary, secondary, accent], neutrals).hex;
...
function deriveThemeFromPrimaryHex(primaryHex, accentHex) {
  const primaryRgb = hexToRgb(primaryHex);
  const accentRgb = hexToRgb(accentHex);
  const primaryHsl = primaryRgb ? rgbToHsl(primaryRgb) : null;
  const accentHsl = accentRgb ? rgbToHsl(accentRgb) : null;
  ...
  if (l > 78 && s < 28) return "light";
  if (h >= 180 && h < 220) return accentHsl && accentHsl.l > 70 ? "frost" : "aquatic";
  if (h >= 220 && h < 255) return "frost";
  if (h >= 255 && h < 300) return "cosmic";
  if (h >= 300 && h < 345) return "shadow";
  if (h >= 345 || h < 16) return "fire";
  if (h >= 16 && h < 60) return l > 55 ? "royal" : "earth";
  if (h >= 60 && h < 110) return "earth";
  if (h >= 110 && h < 160) return "forest";
  if (h >= 160 && h < 200) return s > 45 ? "storm" : "air";
  return "air";
}

function sanitizePaletteRoles(roles) {
  const primary = normalizeColorToHex(roles?.primary);
  const secondary = normalizeColorToHex(roles?.secondary ?? primary);
  const accent = normalizeColorToHex(roles?.accent ?? secondary ?? primary);
  const neutrals = Array.isArray(roles?.neutrals)
    ? roles.neutrals.map((hex) => normalizeColorToHex(hex)).filter(Boolean)
    : [];

  return {
    primary: primary ?? "#2A9D8F",
    secondary: secondary ?? primary ?? "#1D3557",
    accent: accent ?? secondary ?? primary ?? "#E9C46A",
    neutrals: neutrals.length ? neutrals : COHESIVE_NEUTRALS.slice(),
  };
}
        ...
        try {
          ...
          entries.forEach((item) => {
            ...
            collected.push({
              ...
              name,
              icon,
              typeId: Number(item?.typeId ?? item?.type?.id ?? item?.type?.ankamaId) || null,
              palette,
...
function scoreItemPalette(itemPalette, targetPalette, targetSignature) {
  ...
  const normalizedItem = itemPalette.map((hex) => normalizeColorToHex(hex)).filter(Boolean);
  const normalizedTarget = Array.isArray(targetPalette)
    ? targetPalette.map((hex) => normalizeColorToHex(hex)).filter(Boolean)
    : [];
  const targetRgb = normalizedTarget.map((hex) => hexToRgb(hex)).filter(Boolean);
  const itemRgb = normalizedItem.map((hex) => hexToRgb(hex)).filter(Boolean);
  if (!targetRgb.length || !itemRgb.length) {
    return Number.POSITIVE_INFINITY;
  }
  let worstDistance = 0;
  let clusterTotal = 0;
  itemRgb.forEach((color) => {
    let best = Number.POSITIVE_INFINITY;
    targetRgb.forEach((target) => {
      const distance = colorDistance(target, color);
      if (distance < best) {
        best = distance;
      }
    });
    worstDistance = Math.max(worstDistance, best);
    clusterTotal += best;
  });

  const clusterAverage = clusterTotal / itemRgb.length;
  if (worstDistance > 185 || clusterAverage > 150) {
    return Number.POSITIVE_INFINITY;
  }

  const targetCoverage = targetRgb.reduce((acc, target) => {
    let best = Number.POSITIVE_INFINITY;
  ...
  const paletteSignature = buildPaletteSignature(normalizedItem);
  const tonePenalty = toneDistance(paletteSignature, targetSignature);
  if (Number.isFinite(tonePenalty) && tonePenalty > 120) {
    return Number.POSITIVE_INFINITY;
  }

  const coverageScore = targetCoverage / targetRgb.length;
  const toneScore = Number.isFinite(tonePenalty) ? tonePenalty * 2.1 : 0;
  const baseScore = clusterAverage * 0.65 + worstDistance * 0.35 + coverageScore * 0.25 + toneScore;
  return baseScore;
}
...
function chooseItemsForPalette(paletteHex, itemsByType, options = {}) {
  const { anchor, preferred, variantIndex = 0 } = options;
  const result = [];
  ...
  const orderedCoreSlots = ORDERED_SLOTS.filter((slot) => CORE_SLOTS.has(slot));
  const orderedSlots = [
    ...orderedCoreSlots,
    ...Object.keys(itemsByType).filter((slot) => CORE_SLOTS.has(slot) && !orderedCoreSlots.includes(slot)),
  ];
  ...
  orderedSlots.forEach((slot) => {
    ...
    const scored = entries
      ...
      .sort((a, b) => {
        ...
        if (a.ankamaId && b.ankamaId) return a.ankamaId - b.ankamaId;
        return 0;
      });

    const viable = scored.filter((entry) => Number.isFinite(entry.score));
    const closeMatches = viable.filter((entry) => entry.score <= 68);
    ...
    const choice = source[variantIndex % source.length];
    if (choice) {
...
function pickAnchorItem(itemsByType, targetPalette, options = {}) {
  const { variantIndex = 0 } = options;
  ...
  const weighted = candidates
    ...
    .sort((a, b) => {
      ...
      if (a.entry?.ankamaId && b.entry?.ankamaId) {
        return a.entry.ankamaId - b.entry.ankamaId;
      }
      return 0;
    });

  return weighted.length ? weighted[variantIndex % weighted.length]?.entry ?? null : null;
}
...
function pickPalette(rng = createDeterministicRng("palette")) {
  const base = pickDeterministic(HARMONIC_PALETTES, rng);
  const palette = Array.isArray(base) && base.length
    ? base
    : ["#2A9D8F", "#E9C46A", "#F4A261", "#E76F51", "#1D3557", "#F1FAEE"];
  const shifted = palette.map((hex) => {
    const rgb = hexToRgb(hex);
    if (!rgb) {
      return hex;
    }
    const factor = 1 + (rng() * 0.18 - 0.09);
...
function normalizePaletteNumeric(palette) {
  const normalized = Array.isArray(palette)
    ? palette.map((value) => normalizeColorToHex(value)).filter(Boolean)
    : [];
  const seed = normalized.length ? normalized : ["#2A9D8F"];
  while (normalized.length < 6) {
    normalized.push(seed[normalized.length % seed.length]);
  }

  const numeric = normalized.map((value) => {
    const parsed = parseInt(value.replace("#", ""), 16);
    return Number.isFinite(parsed) ? parsed : null;
  });
  return { hex: normalized.slice(0, 6), numeric: numeric.filter((value) => Number.isFinite(value)) };
}
...
function buildSlotPalette(slot, roles) {
  const sanitized = sanitizePaletteRoles(roles);
  const allowed = Array.from(
    new Set([sanitized.primary, sanitized.secondary, sanitized.accent, ...(sanitized.neutrals ?? [])]),
  )
    .map((hex) => normalizeColorToHex(hex))
    .filter(Boolean);

  const neutralDark = allowed.find(
    (hex) => hex === "#111827" || hex === "#1F2937" || hex === "#222222" || hex === "#000000",
  );
  const neutralLight = allowed.find((hex) => hex === "#FFFFFF" || hex === "#F8F9FA" || hex === "#E5E7EB");
  const baseNeutral = neutralDark ?? neutralLight ?? sanitized.primary;
  const metal = neutralLight ?? neutralDark ?? sanitized.secondary;
  const paletteBySlot = {
    coiffe: [sanitized.primary, sanitized.secondary, sanitized.accent, baseNeutral],
    cape: [sanitized.primary, sanitized.secondary, baseNeutral, sanitized.accent],
    bouclier: [sanitized.secondary, sanitized.primary, metal, sanitized.accent],
    costume: [sanitized.primary, sanitized.secondary, sanitized.accent, baseNeutral],
    epauliere: [sanitized.secondary, sanitized.primary, sanitized.accent, baseNeutral],
    ailes: [sanitized.primary, sanitized.accent, sanitized.secondary, baseNeutral],
    familier: [sanitized.primary, sanitized.accent, sanitized.secondary, baseNeutral],
  };
  const palette = paletteBySlot[slot] ?? [sanitized.primary, sanitized.secondary, sanitized.accent, baseNeutral];
  const filtered = palette
    .map((hex) => normalizeColorToHex(hex))
    .filter((hex) => hex && allowed.includes(hex));
  const unique = [];
  ...
  while (unique.length < Math.min(6, allowed.length)) {
    unique.push(allowed[unique.length % allowed.length]);
  }
...
function ensureRequiredSlots(items, itemsByType, paletteHex) {
  const paletteSignature = buildPaletteSignature(paletteHex);
  const usedIds = new Set();
  const bySlot = new Map();

  const register = (item) => {
    if (!item || !item.slot || !CORE_SLOTS.has(item.slot)) return;
    if (usedIds.has(item.ankamaId)) return;
    usedIds.add(item.ankamaId);
    bySlot.set(item.slot, item);
  };

  (Array.isArray(items) ? items : []).forEach(register);

  const pickBestForSlot = (slot, limit = 120) => {
    const entries = Array.isArray(itemsByType[slot]) ? itemsByType[slot] : [];
    if (!entries.length) return null;
    const scored = entries
      .map((item) => ({
        ...item,
        slot,
        score: scoreItemPalette(item.palette ?? [], paletteHex, paletteSignature),
      }))
      .filter((entry) => Number.isFinite(entry.score) && !usedIds.has(entry.ankamaId))
      .sort((a, b) => a.score - b.score);
    const candidate = scored.find((entry) => entry.score <= limit) ?? scored[0];
    return candidate || null;
  };

  REQUIRED_SLOTS.forEach((slot) => {
    if (!bySlot.has(slot)) {
      const candidate = pickBestForSlot(slot, 96);
      if (candidate) {
        register(candidate);
      }
    }
  ...
  OPTIONAL_SLOTS.forEach((slot) => {
    if (!bySlot.has(slot)) {
      const candidate = pickBestForSlot(slot, 110);
      if (candidate) {
        register(candidate);
      }
    }
  ...
  const priorityOrder = [...REQUIRED_SLOTS, ...OPTIONAL_SLOTS];
  return priorityOrder.map((slot) => bySlot.get(slot)).filter(Boolean);
}
...
export function getCreativeToneFilters() {
  return TONE_FILTERS;
}

export function normalizeCreativeColor(input) {
  return normalizeReferenceColor(input);
}

export function pickCreativeSeed(tone) {
  return pickToneSeed(tone);
}

export async function generateGallerySkins({
  ...
  const normalized = normalizeLanguage(language) ?? DEFAULT_LANGUAGE;
  const toneFilter = null;
  const explicitColor = normalizeReferenceColor(color);
  const startIndex = Math.max(0, Math.trunc(offset));
  ...
  for (let index = 0; index < total; index += 1) {
    const referenceColor = explicitColor ?? "#2A9D8F";
    const variantIndex = startIndex + index;
    const rng = createDeterministicRng(`${referenceColor}-${variantIndex}`);
    const breed = pickDeterministic(breeds, rng) ?? breeds[0];
    const gender = rng() > 0.5 ? "f" : "m";
    const creativeRoles = buildCreativePaletteRoles(referenceColor, toneFilter, rng);
    const paletteDesign = buildToneLockedPalette(creativeRoles.palette, creativeRoles.neutrals);
    const paletteNumeric = normalizePaletteNumeric(paletteDesign.hex);

    const recoloredItems = Object.fromEntries(
      Object.entries(itemsByType).map(([slot, entries]) => [slot, applyPaletteRolesToItems(entries, creativeRoles)]),
    );

    const anchor = pickAnchorItem(recoloredItems, paletteNumeric.hex, { variantIndex });
    const selectionPalette = normalizePaletteNumeric(
      anchor?.palette?.length
        ? buildToneLockedPalette([...paletteNumeric.hex, ...anchor.palette], paletteNumeric.hex).hex
        : paletteNumeric.hex,
    );
    const initialItems = chooseItemsForPalette(selectionPalette.hex, recoloredItems, { anchor, variantIndex });
    const aggregatedPalette = aggregatePaletteFromItems(initialItems, selectionPalette.hex);
    const preferred = buildPreferredItemMap(initialItems, aggregatedPalette.hex);
    const selectedItems = chooseItemsForPalette(aggregatedPalette.hex, recoloredItems, {
      anchor,
      preferred,
      variantIndex,
    });
    const ensuredItems = ensureRequiredSlots(selectedItems, recoloredItems, aggregatedPalette.hex);
    const completedPalette = aggregatePaletteFromItems(ensuredItems, aggregatedPalette.hex);
    const harmonizedFinal = buildToneLockedPalette(
      [...completedPalette.hex, ...selectionPalette.hex],
      paletteDesign.hex,
    );
    const cohesivePalette = normalizePaletteNumeric(harmonizedFinal.hex);
    const derivedTheme = deriveThemeFromPrimaryHex(
      cohesivePalette.hex[0],
      cohesivePalette.hex[2] ?? cohesivePalette.hex[1] ?? cohesivePalette.hex[0],
    );
    const theme = ALLOWED_THEMES.includes(derivedTheme) ? derivedTheme : ALLOWED_THEMES[0];
    const faceId = getFaceIdForClass(breed.id, gender);
    skins.push({
      id: `${breed.id}-${variantIndex}-${hashStringToSeed(referenceColor)}`,
      ...
      primaryColor: cohesivePalette.hex[0],
      items: ensuredItems,
    });
  }
  return skins;
