...
export async function generateGallerySkins({ language = DEFAULT_LANGUAGE, count = 9 } = {}) {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const harmonicSeed = pickPalette();
    const paletteDesign = buildDesignerPalette(selectToneSeedColor(harmonicSeed, harmonicSeed));
    const anchor = pickAnchorItem(itemsByType, paletteDesign.palette);
    const selectionPalette = normalizePaletteNumeric(
      anchor?.palette?.length
        ? buildToneLockedPalette([...paletteDesign.palette, ...anchor.palette], paletteDesign.palette).hex
        : paletteDesign.palette,
    );
    const initialItems = chooseItemsForPalette(selectionPalette.hex, itemsByType, { anchor });
    ...
    const selectedItems = chooseItemsForPalette(aggregatedPalette.hex, itemsByType, { anchor, preferred });
    const ensuredItems = ensureRequiredSlots(selectedItems, itemsByType, aggregatedPalette.hex);
