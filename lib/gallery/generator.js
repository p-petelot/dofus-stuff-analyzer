...
function deriveAnalogousPalette(seedHex) {
  ...
  const offsets = [0, 14, -14, 28, -28, 0];
  return offsets.map((shift, index) => {
    const lightAdjust = index === 0 ? 0 : index % 2 === 0 ? -10 : 10;
    const satAdjust = index === offsets.length - 1 ? -12 : -6;
...
function scoreItemPalette(itemPalette, targetPalette, targetSignature) {
  ...
  if (Number.isFinite(tonePenalty)) {
    return baseScore + tonePenalty * 2.1;
  }
  return baseScore;
}
...
function craftHarmonizedPalette(seedPalette, fallbackPalette) {
  ...
  const analogous = deriveAnalogousPalette(base);
  const accentPrimary = baseSource[1] ?? analogous[1];
  const accentSecondary = baseSource[2] ?? analogous[2];
  const luminous = baseSource[3] ?? analogous[3];
  const deep = baseSource[4] ?? analogous[4];
  const muted = baseSource[5] ?? analogous[5];
  const candidates = [
    ...normalizedSeed,
    ...analogous,
    base,
...
function chooseItemsForPalette(paletteHex, itemsByType, options = {}) {
  ...
  orderedSlots.forEach((slot) => {
    ...
    const closeMatches = viable.filter((entry) => entry.score <= 95);
...
function pickAnchorItem(itemsByType, targetPalette) {
  ...
  const weighted = candidates.map((entry) => {
    ...
    const closeness = Number.isFinite(distance) ? Math.max(0.35, (180 - distance) / 180) : 0.55;
    return {
      entry,
      weight: Math.max(1, entry.palette.length) * closeness,
    };
  });
