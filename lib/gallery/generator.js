...
function craftHarmonizedPalette(seedPalette, fallbackPalette) {
  const seed = Array.isArray(seedPalette) ? seedPalette : [];
  ...
  const normalizedFallback = fallback
    .map((value) => normalizeColorToHex(value))
    .filter(Boolean);
  const baseSource = normalizedSeed.length ? normalizedSeed : normalizedFallback;
  const base = baseSource[0] ?? "#2A9D8F";
  const accentPrimary = baseSource[1] ?? adjustHexColor(base, { h: 26 });
  const accentSecondary = baseSource[2] ?? adjustHexColor(base, { h: -26 });
  const luminous = adjustHexColor(base, { l: 14 });
  const deep = adjustHexColor(base, { l: -18 });
  const muted = baseSource[3] ?? adjustHexColor(accentPrimary, { s: -18, l: 6 });
  const candidates = [
    ...normalizedSeed,
    base,
    accentPrimary,
    ...
    deep,
    muted,
    adjustHexColor(base, { h: 48, l: 8 }),
    adjustHexColor(base, { h: -48, l: -6 }),
  ];
  normalizedFallback.forEach((value) => {
  ...
  while (unique.length < 6) {
    const variant = adjustHexColor(base, { h: unique.length * 12, l: unique.length % 2 === 0 ? 18 : -12 });
...
function chooseItemsForPalette(paletteHex, itemsByType, options = {}) {
  ...
  orderedSlots.forEach((slot) => {
    ...
    const closeMatches = viable.filter((entry) => entry.score <= 120);
    ...
    const windowSize = Math.min(source.length, 180);
...
function pickAnchorItem(itemsByType) {
  const candidates = [];
  ...
  const weighted = candidates.map((entry) => ({
    entry,
    weight: Math.max(1, entry.palette.length),
  }));
...
function pickAnchorForPalette(itemsByType, paletteHex) {
  let best = null;
  Object.entries(itemsByType).forEach(([slot, entries]) => {
    if (!Array.isArray(entries)) {
      return;
    ...
    entries.forEach((item) => {
      const palette = Array.isArray(item?.palette)
        ? item.palette.map((value) => normalizeColorToHex(value)).filter(Boolean)
        : [];
      if (!palette.length) {
        return;
      }
      const score = scoreItemPalette(palette, paletteHex);
      if (!Number.isFinite(score)) {
        return;
      }
      if (!best || score < best.score) {
        best = { ...item, slot, palette, score };
      }
    });
  });
  if (best) {
...
function buildPreferredItemMap(items, paletteHex) {
  ...
  items.forEach((item) => {
    const score = scoreItemPalette(item?.palette ?? [], paletteHex);
    if (Number.isFinite(score) && score <= 140 && item?.slot) {
...
function randomBaseColor() {
  ...
  const saturation = 48 + Math.random() * 32;
  const lightness = 32 + Math.random() * 22;
  return hslToHex(hue, saturation, lightness);
}

function buildPaletteFromBaseColor(baseHex) {
  const base = normalizeColorToHex(baseHex) ?? randomBaseColor();
  const analogousA = adjustHexColor(base, { h: 26, s: 6 });
  const analogousB = adjustHexColor(base, { h: -28, s: 6 });
  const deep = adjustHexColor(base, { l: -18, s: 6 });
  const light = adjustHexColor(base, { l: 16, s: -8 });
  const accent = adjustHexColor(base, { h: 142, l: 4 });
  return craftHarmonizedPalette([base, analogousA, analogousB, deep, light, accent], pickPalette());
}
...
export async function generateGallerySkins({ language = DEFAULT_LANGUAGE, count = 9 } = {}) {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const harmonizedSeed = buildPaletteFromBaseColor(randomBaseColor());
    const initialPalette = normalizePaletteNumeric(harmonizedSeed);
    const anchor = pickAnchorForPalette(itemsByType, initialPalette.hex) ?? pickAnchorItem(itemsByType);
