...
function pickToneSeed(tone) {
  ...
  if (candidates.length) {
    return normalizeColorToHex(candidates[Math.floor(Math.random() * candidates.length)]) ?? "#2A9D8F";
  }
  const palette = pickPalette();
  ...
  const choice = pool.length ? pool[Math.floor(Math.random() * pool.length)] : filtered[0];
  return choice?.hex ?? "#2A9D8F";
}

function buildCreativePaletteRoles(referenceHex, toneFilter = null) {
  const reference = normalizeColorToHex(referenceHex) ?? pickToneSeed(toneFilter);
...
export async function generateGallerySkins({
  ...
  const normalized = normalizeLanguage(language) ?? DEFAULT_LANGUAGE;
  const toneFilter = normalizeToneFilter(tone);
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const referenceColor = explicitColor ?? pickToneSeed(toneFilter);
    const creativeRoles = buildCreativePaletteRoles(referenceColor, toneFilter);
