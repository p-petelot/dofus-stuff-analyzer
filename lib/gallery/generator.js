...
function chooseItemsForPalette(paletteHex, itemsByType, options = {}) {
  ...
  orderedSlots.forEach((slot) => {
    ...
    const closeMatches = viable.filter((entry) => entry.score <= 72);
...
export async function generateGallerySkins({ language = DEFAULT_LANGUAGE, count = 9 } = {}) {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const finalPalette = aggregatePaletteFromItems(ensuredItems, aggregatedPalette.hex);
    const paletteRoles = sanitizePaletteRoles(buildCohesivePaletteRoles(finalPalette.hex));
    const recoloredItems = applyPaletteRolesToItems(ensuredItems, paletteRoles);
    ...
    const derivedTheme = deriveThemeFromPrimaryHex(paletteRoles.primary, paletteRoles.accent);
