...
export async function generateGallerySkins({
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const rng = createDeterministicRng(`${referenceColor}-${index}`);
    ...
    const anchor = pickAnchorItem(recoloredItems, paletteNumeric.hex, { variantIndex: index });
    ...
    const initialItems = chooseItemsForPalette(selectionPalette.hex, recoloredItems, {
      anchor,
      variantIndex: index,
    });
    ...
    skins.push({
      id: `${breed.id}-${index}-${hashStringToSeed(referenceColor)}`,
      number: index + 1,
      classId: breed.id,
