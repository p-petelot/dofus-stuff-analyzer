...
const REQUIRED_SLOTS = ["coiffe", "cape", "bouclier"];
...
function deriveThemeFromPrimaryHex(primaryHex, accentHex) {
  ...
  if (h >= 180 && h < 225) return accentHsl && accentHsl.l > 70 ? "frost" : "aquatic";
  if (h >= 225 && h < 255) return "frost";
  if (h >= 255 && h < 300) return "cosmic";
  if (h >= 300 && h < 345) return "shadow";
  if (h >= 345 || h < 16) return "fire";
  if (h >= 16 && h < 55) return l > 55 ? "royal" : "earth";
  if (h >= 55 && h < 95) return "earth";
  if (h >= 95 && h < 135) return "forest";
  if (h >= 135 && h < 170) return "nature";
  if (h >= 170 && h < 180) return "wind";
  return "air";
}
