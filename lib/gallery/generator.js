...
export async function generateGallerySkins({ language = DEFAULT_LANGUAGE, count = 9 } = {}) {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const anchor = pickAnchorItem(itemsByType);
    const basePaletteValues = Array.isArray(anchor?.palette) && anchor.palette.length ? anchor.palette : pickPalette();
    const normalizedBase = Array.isArray(basePaletteValues)
      ? basePaletteValues.map((value) => normalizeColorToHex(value)).filter(Boolean)
      : [];
    const harmonizedSeed = craftHarmonizedPalette(normalizedBase, pickPalette());
