...
function chooseItemsForPalette(paletteHex, itemsByType, options = {}) {
  ...
  Object.entries(itemsByType).forEach(([slot, entries]) => {
    ...
    const windowSize = Math.min(source.length, 64);
...
function aggregatePaletteFromItems(items, fallbackPalette) {
  ...
  return normalizePaletteNumeric(ordered.slice(0, 6));
}
...
export async function generateGallerySkins({ language = DEFAULT_LANGUAGE, count = 9 } = {}) {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const normalizedBase = Array.isArray(basePaletteValues)
      ...
      : [];
    const seedValues = normalizedBase.length ? normalizedBase : pickPalette();
    const paletteSeed = Array.isArray(seedValues) && seedValues.length ? seedValues : ["#2A9D8F", "#E9C46A", "#F4A261", "#E76F51", "#1D3557", "#F1FAEE"];
    const initialPalette = normalizePaletteNumeric(paletteSeed);
