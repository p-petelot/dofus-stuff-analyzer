...
/** @typedef {import("../types").PaletteSwatch} PaletteSwatch */
...
/**
 * @param {Object.<FourSlot, Mask>} [masks]
 * @returns {(x: number, y: number) => boolean}
 */
function ensureMaskMatch(masks) {
  if (!masks) {
    return () => true;
  }
 ...
 * @param {(x: number, y: number) => boolean} predicate
 * @returns {{ buckets: Map<string, { r: number; g: number; b: number; count: number }> }}
 */
function buildBuckets(img, predicate) {
...
/**
 * @param {Map<string, { r: number; g: number; b: number; count: number }>} buckets
 * @returns {PaletteSwatch[]}
 */
function finalizePalette(buckets) {
  const swatches = [];
...
export function deltaE2000(c1, c2) {
  ...
  const meanHPrime =
    Math.abs(h1Prime - h2Prime) > deg180 ? (h1Prime + h2Prime + deg360) / 2 : (h1Prime + h2Prime) / 2;
  ...
  const deltaTheta =
    degreesToRadians(30) * Math.exp(-Math.pow((meanHPrime - degreesToRadians(275)) / degreesToRadians(25), 2));
